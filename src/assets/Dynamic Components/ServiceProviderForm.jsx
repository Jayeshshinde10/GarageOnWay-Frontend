...
const ServiceProviderForm = ({latitude,longitude}) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateForm()) {
      try {
        // Submit form data
        const response = await axios.post('http://127.0.0.1:8000/ServiceProvider/',{
          image1: formData.image1,
          image2: formData.image2,
          orginazation_name: formData.organization_name,
          closing_time: formData.closing_time,
          opening_time: formData.opening_time,
          near_by_landmark: formData.near_by_landmark,
